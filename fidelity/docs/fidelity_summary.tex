...
\section{Environmental Complexity and Scenario Comparison}
To evaluate the scalability of our fidelity findings, we contrast the \texttt{simple\_street\_canyon} (a toy scene with 2 building rows) against a dense urban model of \texttt{munich} (332 buildings). 

\subsection{Methodology for Scale-Up}
Per reliability requirements, both scenarios use a consistent 441-user grid. For the Munich scenario, we optimized the ray-tracing runner by increasing the batch size to 64 and utilizing 100K rays per source, achieving high-density statistical trends in a complex environment.

\section{Results and Observations}
We present side-by-side comparisons (Simple vs. Munich) to identify how environmental complexity amplifies or mitigates fidelity impacts.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{../results/geometry_comparative.png}}
\caption{Geometry Sensitivity (Comparative): Impact of Coordinate Noise on Simple (Top) vs. Munich (Bottom).}
\label{fig-geom}
\end{figure}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{../results/material_comparative.png}}
\caption{Material Sensitivity (Comparative): Impact of Simplifications on Simple (Top) vs. Munich (Bottom).}
\label{fig-mat}
\end{figure}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{../results/ray_tracing_comparative.png}}
\caption{Ray Tracing Expansion (Comparative): Reflection Depth and Ray Count sensitivity for Simple (Top) vs. Munich (Bottom).}
\label{fig-rt}
\end{figure}
